...
```{r}
exist_list$Meta_data["PAM"] <- "Pre-active Microglia"
m <- as.data.table(exist_list$Meta_data[-c(3,1,7)], keep.rownames = TRUE) 
setnames(m, c("Acronym","Full Name"))
m
```
...
presentVln <- VlnPlot(seurat_combi, features = presentMarker, group.by = "orig.ident", cols = pal_simpsons()(16),pt.size = 0, ncol = 10) 
presentRdg <- RidgePlot(seurat_combi, features = presentMarker, group.by = "orig.ident", cols =  pal_simpsons()(16), ncol = 10) 
...
```{r plot the last DEG HMP}
sce_labelCaliAll <- readRDS("../Annotated_sce.rds")
sce_labelCaliAll <- scuttle::logNormCounts(sce_labelCaliAll)
# Get both KO and WT subset
sce_labelCaliWT <- sce_labelCaliAll[, sce_labelCaliAll$orig.ident == "WT"]
sce_labelCaliKO <- sce_labelCaliAll[, sce_labelCaliAll$orig.ident != "WT"]

# Apply and visualize on both of them
sce_labelCali_agg <- muscat::aggregateData(sce_labelCaliKO, assay = "logcounts", by = "label_calibrated", fun = "mean")
assayNames(sce_labelCali_agg) <- "logcounts"
sechm(sce_labelCali_agg, features = as.character(keyGenes), assayName = "logcounts", show_rownames = TRUE, show_colnames = TRUE, do.scale = TRUE, 
      hmcols = viridis_pal(option = "C")(100), 
      sortRowsOn = NULL, cluster_rows = FALSE)
```
```{r}
GPNMB_Exp <-  as.data.frame(assay(sce_labelCaliAll, "logcounts")["Gpnmb",])
colnames(GPNMB_Exp) <- "GPNMB"
GPNMB_Exp$Sample <- sce_labelCaliAll$orig.ident[match(rownames(GPNMB_Exp), rownames(colData(sce_labelCaliAll)))]
GPNMB_Exp$Label <- sce_labelCaliAll$label_calibrated[match(rownames(GPNMB_Exp), rownames(colData(sce_labelCaliAll)))]
#GPNMB_Exp <- GPNMB_Exp[GPNMB_Exp$GPNMB>0, ]

ggplot(GPNMB_Exp, aes(x = Sample, y = GPNMB, fill = Sample)) + geom_violin() + ylab("GPNMB logcounts")
```
