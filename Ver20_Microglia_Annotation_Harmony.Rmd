...
working_path <- "/Users/jiesun/Storage/Work/Projects/PAST/Microglial_annotation/Microglial-annotation"
# setwd(working_path)
```
...
plotDataComposition <- function(seruatObject, classLabel, dataLabel, colorOption = "G") {
  ...
  ca[, percent := 100*(N/sum(N)), by = sample][, cluster := as.factor(cluster)]
...
(plot_clusterComp <- plotDataComposition(seruatObject = seurat_combi, classLabel = "seurat_clusters", dataLabel = "orig.ident") + theme_minimal())
```
...
if (old_way_loading){
  ...
  marker_list <- readxl::read_excel(file.path(working_path,"Markers/Marker_list_BU.xlsx"))[-8]
  
  Meta_Data <- unlist(readxl::read_excel(file.path(working_path,"Markers/Marker_list_BU.xlsx"))[8])
...
loA <- list(WT = LayerData(seurat_combi, "data.WT"), 
            G1 = LayerData(seurat_combi, "data.GpnmbKO_1"), 
            G2 = LayerData(seurat_combi, "data.GpnmbKO_2"))
logcounts <- Reduce(cbind, loA[-1], loA[[1]])
...
cellType_calibrated[c("2", "6")]   <- if (acronym==TRUE) c("PAM") else c("Pre-active microglia")
...
seurat_sce$label_auto <- cellType_auto[seurat_sce$seurat_clusters] %>% as.character()
seurat_sce$label_cali <- cellType_calibrated[seurat_sce$seurat_clusters] %>% as.character()
...
seurat_combi$label_auto <- as.character(cellType_auto[seurat_sce$seurat_clusters])
seurat_combi$label_calibrated <- as.character(cellType_calibrated[seurat_sce$seurat_clusters])
...
plot_labelCali <- DimPlot(seurat_combi, group.by = "label_calibrated",pt.size = 0.3, cols = pal_futurama("planetexpress")(12)[-c(1)], label = TRUE) + 
...
if (saving == TRUE) {
  write.csv(topMatLab, file.path(working_path,sprintf("TOP_%s_gene_per_matrix.csv", top_number)))
}
```
...
presentVln <- VlnPlot(seurat_combi, features = presentMarker, group.by = "label_calibrated", cols = pal_simpsons()(16),pt.size = 0, ncol = 7) 
presentRdg <- RidgePlot(seurat_combi, features = presentMarker, group.by = "label_calibrated", cols =  pal_simpsons()(16), ncol = 7) 

if (saving == TRUE) {
  pdf("Visualization/Marker_Vln_labCal.pdf", width = 30, height =18)
  print(presentVln)
  print(presentRdg)
  dev.off()

  pdf("Visualization/Marker_Rdg_labCal.pdf", width = 30, height = 8)
  print(presentRdg)
  dev.off()  
}
