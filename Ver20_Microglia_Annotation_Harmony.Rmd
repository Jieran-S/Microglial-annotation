...
seurat_combi <- FindNeighbors(seurat_combi, reduction = "harmony", dims = 1:pcs)
seurat_combi <- FindClusters(seurat_combi, resolution = 0.6) # used to be 0.5
...
if (old_way_loading){
  ...
  exist_list <- readRDS(file.path(working_path, "Markers/marker_list.rds"))
...
seurat_combi$cluster_auto <- cellType_auto[seurat_sce$ident]
seurat_combi$cluster_calibrated <- cellType_calibrated[seurat_sce$ident]
...
plot_labelAuto <- DimPlot(seurat_combi, group.by = "cluster_auto",pt.size = 0.3, cols = pal_npg("nrc")(10), label = TRUE) + 
...
links <- data.table(table(source=unlist(paste0("Cluster_", seurat_combi$seurat_clusters)), 
                             target=unlist(seurat_combi$cluster_calibrated))) %>% 
  .[N != 0,] %>% .[order(-N),]
...
(plot_cellTypeCompCali <- plotDataComposition(seruatObject = seurat_combi, classLabel = "cluster_calibrated", dataLabel = "orig.ident"))
```
...
VPLab_gene <- VlnPlot(seurat_combi, features = featured_gene, 
                   group.by = "cluster_calibrated", cols = pal_futurama("planetexpress")(12) ,pt.size = 0) + 
            xlab("Cell type") 
# ridge plot
...
RPLab_gene <- RidgePlot(seurat_combi, features = featured_gene, group.by = "cluster_calibrated", 
...
```{r Return significantly expressed value}
Target_celltype <- names(exist_list)[unlist(lapply(exist_list, function(x) featured_gene %in% x))]

seruat_targetcell <- as.numeric(seurat_combi[["RNA"]][featured_gene,])

WTExpression <- data.frame(Expression = seruat_targetcell[which(seurat_combi$orig.ident == "WT" & 
                                          seurat_combi$cluster_calibrated == Target_celltype)],
                           Condition = "WT")

KOExpression <- data.frame(Expression = seruat_targetcell[which(seurat_combi$orig.ident == "GpnmbKO" & 
                                          seurat_combi$cluster_calibrated == Target_celltype)],
                           Condition = "GpnmbKO")


CompareDF <- rbindlist(list(WTExpression, KOExpression))

(Barplot_gene <- ggplot(CompareDF, aes(x=Condition, y=Expression, fill = Condition)) + 
  geom_boxplot() +
  geom_signif(comparisons = list(c("WT", "GpnmbKO")), 
