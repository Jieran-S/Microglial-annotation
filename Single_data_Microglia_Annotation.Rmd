...
seurat_combi <- Read10X_h5(file.path(data_path, "filtered_feature_bc_matrix.h5")) %>%
  CreateSeuratObject(project = "filtered", min.cells = 3, min.features = 200) 

seurat_combi[["percent.mt"]] <- PercentageFeatureSet(seurat_combi, pattern = "^mt-")
seurat_combi[["percent.rb"]] <- PercentageFeatureSet(seurat_combi, pattern = "^Rp[sl]")
seurat_combi %>% 
...
ElbowPlot(seurat_combi, ndims= 50) + ylab("variance explained (%)") +
                ...
                geom_label(
                  ...
                  x=pcs,
                  y=seurat_combi[["pca"]]@stdev[pcs] + 3,
...
# Harmony integration
seurat_combi <- RunHarmony(seurat_combi, dims.use = 1:pcs, max.iter.harmony = 50) %>% 
    RunUMAP(seurat_combi, reduction = "harmony", dims = 1:pcs, reduction.name = 'har_umap',reduction.key="UMAPHAR_") %>% 
    FindNeighbors(seurat_combi, reduction = "harmony", dims = 1:pcs, graph.name = 'har_snn') %>% FindClusters(resolution = 1)
...
Plot1 <- DimPlot(seurat_combi, group.by = "seurat_clusters", label = T, label.color = "black", pt.size = 0.3, cols = pal_npg("nrc")(10))  + 
  plot_annotation(title="Clusters") + NoAxes()
Plot2 <- DimPlot(seurat_combi, group.by = "orig.ident",pt.size = 0.3, cols = pal_npg("nrc")(10)) + 
...
DoHeatmap(seurat_combi, features = seurat_markers$gene, group.colors = pal_npg("nrc")(8))
```
