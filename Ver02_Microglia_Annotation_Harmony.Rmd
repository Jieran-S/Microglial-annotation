...
working_path <- "/mnt/plger/jiesun/TFCrossFactor/MGCluster/Microglial-annotation" 
setwd(working_path)
```
...
```
```{r show heatmap row names}
(heatmap_showrowname <- sechm::sechm(pb, unlist(exist_list$Unique_IRM), 
             assayName = "logcounts", gaps_row = "marker4", 
             show_colnames = TRUE, do.scale = TRUE, 
             breaks=1, row_title_rot=0, show_rownames = TRUE, 
             hmcols = viridis_pal(option = "C")(100)))

(heatmap_showrowname <- sechm::sechm(pb, unlist(c(exist_list$Unique_DAM, exist_list$Unique_ATM)), 
             assayName = "logcounts", gaps_row = "marker4", 
             show_colnames = TRUE, do.scale = TRUE, 
             breaks=1, row_title_rot=0, show_rownames = TRUE, 
             hmcols = viridis_pal(option = "C")(100)))
...
top_number <- 15
top_matrix <- data.table(assay(pb)[unlist(exist_list),], keep.rownames = TRUE)
...
write.csv(top_matrix, sprintf("TOP_%s_gene_per_matrix.csv", top_number))
...
plot_labelled <- DimPlot(seurat_combi, group.by = "cluster_label",pt.size = 0.3, cols = pal_npg()(10)[-1], label = TRUE) + 
...
saving = FALSE
if (saving){
  saveRDS(seurat_combi, "Annotated_seurat.rds")
}
```
...
heatmapmarkersFinal<-c("P2ry12","Cx3cr1","P2ry13","Tmem119","Csf1r","Hexb","Mrc1","Ms4a7","Pf4","Cd9","Trem2","Spp1","Itgax","Cd83","Ifit1","Ifit2","Isg15","Ifitm3","Ccl3","Ccl4","Cst7","Il1b","Rtp4","Cd68","Ctsd","Lamp1","Usp18","Cdk1","Mki67","Birc5","Cd36","Cd74","H2-Aa","Cdkn2a","Cdkn1a","Cdkn2d","Casp8","Il1b","Glb1","Serpine1","Il1rl1","HOXB8")

featured_gene <- unlist(exist_list)[1]
feature_list <- list(flist_1, flist_2, flist_3, flist_4, flist_5, flist_6, flist_7, flist_8, heatmapmarkersFinal)
...
```{r saving a pdf}
pdf("Results/Annotation_Harmony.pdf", width = 15, height = 9)
print(VP_Overall)
print(plot_UMAP)
