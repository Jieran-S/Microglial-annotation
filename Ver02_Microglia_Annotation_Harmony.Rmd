...
seurat_combi <- subset(seruat_combi, subset = nFeature_RNA > 200 & nFeature_RNA < 10000 & percent.mt < 7 & percent.rb < 10 ) 
...
feature_list <- list(flist_1, flist_2, flist_3, flist_4, flist_5, flist_6, flist_7, flist_8)
```
...
```{r plot required vln plot & ridge plot}
plotMarkerList <- function(feature_list, group_var, plot_type = c("Vln", "Rdg"), saving = FALSE){
  ...
  if (saving == TRUE) {
    label <- strsplit(group_var, c("_"))[[1]][2]
...
```{r Vln and ridge plot for one gene}
# Vlnplot
VP_gene <- VlnPlot(seurat_combi, features = flist_7, 
